...
<div class="m-t-25 table-responsive">
    <table id="table2" class="table table-hover table-bordered datatable-minimal">
        ...
        <tbody>
            @php $total = 0; @endphp
            @php $no = 1; @endphp
            @foreach ($datarealisasiuser1 as $d )
                <tr>
                    <td>{{ $no++ }}</td>
                    <td>{{ $d->no_rekening }}</td>
                    <td>{{ $d->rekening2 }}</td>
        ...
        <tfoot>
            <tr>
                ...
                <td style="text-align: right"><strong> {{ number_format($total = $datarealisasiuser1->sum('nilai_transaksi'), 0) }}</td>
            </tr>
        </tfoot>
    </table>
</div>
